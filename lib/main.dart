  ...
  Widget build(BuildContext context) => Scaffold(
    body: SafeArea(child: Center(child: SingleChildScrollView(padding: const EdgeInsets.all(24), child: ConstrainedBox(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
        ...
        Column(crossAxisAlignment:CrossAxisAlignment.start,children:[InputDecorator(decoration:InputDecoration(labelText:L10n.t('Language')),child:DropdownButtonHideUnderline(child:DropdownButton<String>(value:AppLocale.code,isExpanded:true,menuMaxHeight:520,items:AppLanguages.supported.map((l)=>DropdownMenuItem<String>(value:l.code,enabled:true,child:Text(l.nativeName,style:const TextStyle(fontSize:16)))).toList(),onChanged:(v){if(v==null)return;AppLocale.set(v);setState((){});L10n.warmLanguage(v).then((_){if(mounted)setState((){});});}))),const SizedBox(height:6),LText('Choose your language. The selected language is applied immediately; supported translation models continue loading in the background.',style:const TextStyle(fontSize:11,color:earth))]),
