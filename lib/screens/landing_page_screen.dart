...
class LandingPageScreen extends StatelessWidget {
  ...
  void _save(BuildContext context) {
    ...
    ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Product page saved to your Karigar Setu shop.')));
  }

  @override
  Widget build(BuildContext context) => Scaffold(
        backgroundColor: const Color(0xFFF7F0E7),
        appBar: AppBar(title: const Text('Your Product Page'), actions: [IconButton(onPressed: () => _share(context), icon: const Icon(Icons.share_rounded))]),
        body: ListView(
          ...
          children: [
            ...
            Container(
              ...
              child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                ...
                Text(generated.title, style: const TextStyle(fontSize: 29, height: 1.08, fontWeight: FontWeight.w900)),
                const SizedBox(height: 8),
                Text(generated.tagline, style: const TextStyle(fontSize: 16, color: AppColors.terracottaDark, fontWeight: FontWeight.w600, height: 1.4)),
                ...
                Container(padding: const EdgeInsets.all(18), decoration: BoxDecoration(color: AppColors.cream, borderRadius: BorderRadius.circular(18), border: Border.all(color: AppColors.gold.withOpacity(.35))), child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [const Icon(Icons.format_quote_rounded, color: AppColors.gold), const SizedBox(width: 10), Expanded(child: Text(generated.artisanNote, style: const TextStyle(fontStyle: FontStyle.italic, height: 1.45)))])),
                const SizedBox(height: 24),
                Row(children: [Expanded(child: ElevatedButton.icon(onPressed: () => _share(context), icon: const Icon(Icons.share_rounded), label: const Text('Share Page'))), const SizedBox(width: 10), IconButton.filled(onPressed: () => _save(context), icon: const Icon(Icons.bookmark_add_outlined), tooltip: 'Save')]),
                const SizedBox(height: 14),
                Center(child: Text('Powered by Karigar Setu • AI-assisted storytelling', style: TextStyle(fontSize: 11, color: Colors.black.withOpacity(.45))),),
              ]),
            ),
          ],
        ),
